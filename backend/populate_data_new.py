    ...
    try:
        ...
        if not emp.user:
            user = User.objects.create_user(
                username=emp_data['cedula'],
                email=emp_data['email'],
                password=emp_data['cedula'],  # Contraseña = cédula
            ...
            emp.user = user
            emp.save()
            print(f"   ✅ Creado: {emp.nombres} {emp.apellidos} ({emp_data['cedula']}) - {role}")
        else:
...
print("   Empleados: use su cédula como usuario y contraseña")
print("=" * 60)
