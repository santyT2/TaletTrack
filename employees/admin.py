...
class CargoAdmin(admin.ModelAdmin):
    ...
    """
    list_display = ('nombre', 'nivel_requerido', 'salario_base', 'created_at')
    list_filter = ('nivel_requerido', 'created_at')
...
class EmpleadoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Información Personal', {
            'fields': ('nombres', 'apellidos', 'cedula', 'fecha_nacimiento', 'foto')
        }),
        ('Contacto', {
            'fields': ('email', 'telefono')
        }),
        ('Información Laboral', {
            'fields': ('cargo', 'sucursal', 'fecha_ingreso', 'estado')
        }),
        ('Auditoría', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
